...
class RobotController:
    def __init__(self, bt_addr, camera_index=0, resolution=(1280, 720), camera_height=300):
        ...
        # CAMBIO 1: Ahora el frente está a +90° (perpendicular) del ArUco y a 20cm de distancia
        self.robot_front_offset = 10  # 20 cm de distancia
        self.robot_front_angle = math.pi/2  # +90 grados (perpendicular) respecto al eje X del aruco
        ...
        self.distance_tolerance = self.distance_target*0.1  # Tolerancia de 10% de 5cm = 0.5cm
        # Agregar tolerancia de alineación del 10% para el punto objetivo
        self.target_alignment_tolerance = math.radians(18)  # 10% de 180 grados (π radianes) es aproximadamente 18 grados
        ...
        # Velocidad máxima para conversión PWM
        self.max_speed = 10.0  # Añadido: valor máximo para escalar a PWM
    ...
    def move_robot(self, x, y, theta, x_target, y_target):
        ...
        """
        # Ajustamos la posición del robot para considerar el offset del frente rotado 90° CCW
    ...
    def print_status(self, force=False):
        ...
        if force or (current_time - self.last_print_time >= self.print_interval):
            ...
            # Añadir offset del frente (corregido con rotación)
    ...
    def draw_robot_front(self, frame, robot_x, robot_y, theta):
        ...
        # Ajuste: Rotar 90° counterclockwise añadiendo -pi/2 al ángulo
        front_length = abs(self.robot_front_offset) / self.pixel_to_cm  # Convertir cm a píxeles
        front_x = int(abs_x + front_length * math.cos(theta - math.pi/2))
        front_y = int(abs_y + front_length * math.sin(theta - math.pi/2))
...
def main():
    ...
    controller = RobotController(
        ...
        camera_index=0,
        resolution=(1280, 720),
        camera_height=200  # Altura de la cámara en cm
    )
